  ...
  app.post('/api/signup', function(req, res){
    ...
    User.findOne({where:{'email':email}}).then(function(user) {
      ...
      return new Promise(function(yell, cry) {
        req.logIn(data, function(err) { if(err) { cry(err) } else { yell(data) } })
      });
  ...
  app.post('/api/login',  function(req, res){
    new Promise(function(resolve, reject) {
      passport.authenticate('local', function(err, user, info){
        if(err || !user) { reject(new Error("Bad username or password")) } else { resolve(user) }
      })(req, res);
    }).then(function(user) {
      return new Promise(function(yell, cry) {
        req.logIn(user, function(err) { if(err) { cry(err) } else { yell(user) } })
      });
    }).then(function(user) {
      res.json({
        auth:true,
